...
public class ListadodeProveedores extends JDialog {
					...
					table.addMouseListener(new MouseAdapter() {
						...
						public void mouseClicked(MouseEvent arg0) {
							...
							if(seleccion!=-1) {
								...
								p = Empresa.getInstance().buscarProveedor((String)modelo.getValueAt(modelrow, 0));
					...
					btnPagar.addActionListener(new ActionListener() {
						public void actionPerformed(ActionEvent e) {
							...
							if(opcion==JOptionPane.OK_OPTION) {
								Empresa.getInstance().PagarDeudaProveedor(p);
	...
	private void cargarTabla() {
		...
		for(Proveedor provee : Empresa.getInstance().getMisproveedores()){
			
				fila[0] = provee.getRnc();
				fila[1] = provee.getNombre();
				fila[2] = provee.getTelefono();
				fila[3] = provee.getDireccion();
				fila[4] = provee.getCuentapagar(); 
				modelo.addRow(fila);
			}
